...
1. `tickets.next({stage: "FRONTEND"})` — discover available tickets.
2. `tickets.claim({ticket_id, agent: "Frontend", machine_id, operator})` — acquire distributed lock.
3. Execute UI implementation (git two-commit protocol still applies for code artifacts).
4. `tickets.complete({ticket_id, evidence: {artifacts, a11y_audit, coverage}})` — advance to QA stage.
...
```bash
python3 .github/tickets.py --claim <id> Frontend $(hostname) <operator>
# ... execute work ...
python3 .github/tickets.py --advance <id> Frontend
```
