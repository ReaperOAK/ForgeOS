...
if [[ ! -f "${ENV_FILE}" ]]; then
  if [[ -f "${REPO_ROOT}/.env.example" ]]; then
    ...
    cat > "${ENV_FILE}" <<- 'ENVEOF'
		# ForgeOS — Admin token (generated by setup.sh)
...
# Replace the default placeholder token with a secure random one if it's still default
if grep -q 'FORGEOS_ADMIN_TOKEN=forgeos_admin_CHANGE_ME' "${ENV_FILE}" 2>/dev/null; then
  NEW_TOKEN=$(openssl rand -hex 32 2>/dev/null || python3 -c "import secrets; print(secrets.token_hex(32))" 2>/dev/null || echo "forgeos_admin_CHANGE_ME")
  if [[ "$NEW_TOKEN" != "forgeos_admin_CHANGE_ME" ]]; then
    if [[ "$(uname)" == "Darwin" ]]; then
      sed -i '' "s/FORGEOS_ADMIN_TOKEN=forgeos_admin_CHANGE_ME/FORGEOS_ADMIN_TOKEN=$NEW_TOKEN/" "${ENV_FILE}"
    else
      sed -i "s/FORGEOS_ADMIN_TOKEN=forgeos_admin_CHANGE_ME/FORGEOS_ADMIN_TOKEN=$NEW_TOKEN/" "${ENV_FILE}"
    fi
    ok "Generated secure FORGEOS_ADMIN_TOKEN in .env"
  else
    warn "Could not generate secure token — install openssl or python3. Keeping placeholder."
  fi
else
  ok "FORGEOS_ADMIN_TOKEN already set in .env (not the default)"
fi
...
printf "${GREEN}║                                                          ║${RESET}\n"
printf "${GREEN}║  Your FORGEOS_ADMIN_TOKEN has been generated in .env.    ║${RESET}\n"
printf "${GREEN}║  VS Code will pick it up from your shell environment.    ║${RESET}\n"
printf "${GREEN}║                                                          ║${RESET}\n"
