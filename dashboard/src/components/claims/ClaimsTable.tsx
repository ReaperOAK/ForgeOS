          ...
          <tr className="border-b border-border">
            {COLUMNS.map((col) => (
              <th
                ...
                className={`${col.className} text-left text-xs font-medium text-muted uppercase tracking-wide px-3 py-3 cursor-pointer hover:text-foreground select-none focus-ring ${
                  col.field === sortField ? 'text-primary' : ''
                }`}
                tabIndex={0}
          ...
          {isLoading ? (
            ...
            sortedClaims.map((claim, idx) => {
              ...
              return (
                <tr
                  key={claim.ticketId}
                  className={`border-b border-border transition-colors ${ROW_STATE_CLASSES[rowState]} ${
                    rowState === 'normal' ? stripeBg : ''
                  }`}
                >
