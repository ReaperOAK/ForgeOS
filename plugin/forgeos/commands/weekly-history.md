...
in `.forgeos/prompts/weekly-history.prompt.md` — read that file in its entirety and follow it step by step.

First run the safety boot: read `.forgeos/guardian/STOP_ALL` (halt if it contains
`STOP`), then `.forgeos/AGENTS.md` and the relevant `.forgeos/instructions/*.instructions.md`.
